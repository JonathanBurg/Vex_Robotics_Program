...
/*                                                                            */
/*    Module:       main.cpp                                                  */
/*    Author:       Bennett Kidder                                            */
/*                                                                            */
/*    Created:      Wed SEP 26 2022                                           */
/*    Description:  Competition Auton for the spin up comp                    */
/*                                                                            */
/*----------------------------------------------------------------------------*/
...
void brainGUI(){
  int RT = (RightSideMotorA.temperature(percent)+ RightSideMotorB.temperature(percent) + RightSideMotorC.temperature(percent)) / 3;
  int LT = (LeftSideMotorA.temperature(percent)+ LeftSideMotorC.temperature(percent)+ LeftSideMotorC.temperature(percent)) / 3;
...
// these do things such as driving the whole bot forward or left or whatever the name says. 
// its a ton easier than typing more than neccessary
...
void autonomous(void) {
  ...
  RotationL.setPosition(0,turns);
  sleep(100);
  // at this point the cylinder has been spun and we should be parallel with the white lines (WORKING)
  ...
  Lv = 40;
  Rv = 40;
  waitUntil(RotationR.position(turns) <= -7);
  SidesSTP();

  if(RotationR.position(turns) < -7.1){
    Lv = 25;
    Rv = 25;
    SidesREV();
    waitUntil(RotationR.position(turns) == -7);
  ...
  if(RotationR.position(turns) > -6.9){
    Lv = 25;
    Rv = 25;    
    SidesFWD();
    waitUntil(RotationR.position(turns) == -7);
  ...
  RightSideREV();
  waitUntil(RotationR.position(turns) <= -2.05);
  SidesSTP();

  if (RotationR.position(turns) < -2.1){
    Lv = 25;
    Rv = 25;
    RightSideFWD();
    waitUntil(RotationR.position(turns) == -2.05);
  ...
  if (RotationR.position(turns) > -1.9){
    Lv = 25;
    Rv = 25;
    RightSideREV();
    waitUntil(RotationR.position(turns) == -2.05);
  ...
  LeftSideREV();
  waitUntil(RotationL.position(turns) <= -.1);
  Lv = 60;
  Rv = 60;
  waitUntil(RotationL.position(turns) <= -.6);
  Lv =50;
  Rv=50;
  waitUntil(RotationL.position(turns) <= -1);
  SidesSTP();
